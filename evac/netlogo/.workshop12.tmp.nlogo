...
globals [
   max-food-amount
   cost-of-offspring
   ; daily-cost-of-living ;; not need as it is set through the interface
]
...
to share  ;; turtle procedure
  if any? other turtles [
    ...
    [ ask nearest-turtle [ set energy energy - aid ]
      ...
      face nearest-turtle
      create-link-to nearest-turtle ;link pointing at the *donor*  (use create-link-from to point the other way)
    ]
  ]
end

to-report share-communism [energyOther]
  let aid (energyOther - energy) / 2   ;code for sharing
