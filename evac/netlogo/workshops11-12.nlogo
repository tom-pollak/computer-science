...
globals [
   max-food-amount
   cost-of-offspring
   ; daily-cost-of-living ;; not needed as it is set through the interface
]
...
to setup
  ...
  create-turtles initial-turtles [
    set energy (1 + random 20)
    set chromosome n-values 8 [random 2]
...
to go
  ...
  ask turtles [
    compute-f3
    ; show list energy f3 ;; uncomment to see raw fitness ('energy')
                          ;; next to f3 fitness for the same individual
  ]
  ask turtles [ eat ]
  ask turtles [ move ]
...
to compute-f3
  ...
  repeat length chromosome [     ; for each locus (aka position) on the chromosome...
    ...
    let tmp energy               ; to sum up and average fitnesses
    ask other turtles with [item i chromosome = gene] [
      set tmp tmp + energy       ; energy is the fitness of a relevant turle from the other ones
...
to share  ;; turtle procedure
  if any? other turtles [
    let nearest-turtle min-one-of other turtles [distance myself]
    let energy2 [energy] of nearest-turtle
    let aid (energy2 - energy) / 2   ;code for sharing
    if aid > 0                       ;code for sharing
      [ ask nearest-turtle [ set energy energy - aid ]
        set energy energy + aid
        face nearest-turtle
        create-link-to nearest-turtle ;link pointing at the *donor*
                                      ; (use create-link-from to point the other way)
      ]
  ]
end
...
; Public Domain:
; To the extent possible under law, Uri Wilensky and Dimitar Kazakov have waived all
