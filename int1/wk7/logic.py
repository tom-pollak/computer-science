...
def applySubst(form, subst):
    ...
    if form.isa(Constant):
        return form
    if form.isa(Atom):
        return Atom(*[form.name] + [applySubst(arg, subst) for arg in form.args])
    if form.isa(Not):
        return Not(applySubst(form.arg, subst))
    if form.isa(And):
        return And(applySubst(form.arg1, subst), applySubst(form.arg2, subst))
    if form.isa(Or):
        return Or(applySubst(form.arg1, subst), applySubst(form.arg2, subst))
...
class ToCNFRule(UnaryRule):
    ...
    def applyRule(self, form):
        ...
        conjuncts = [
            OrList(reduceFormulas(flattenOr(f), Or)) for f in flattenAnd(newForm)
        ]
        ...
        for i, item1 in enumerate(items1):
            for j, item2 in enumerate(items2):
                ...
                if unify(negateFormula(item1), item2, subst):
                    ...
                    newItems = [
                        applySubst(item, subst) for item in newItems1 + newItems2
                    ]
...
def interpretForm(form, model):
    ...
    if form.isa(Not):
        return not interpretForm(form.arg, model)
    if form.isa(And):
        return interpretForm(form.arg1, model) and interpretForm(form.arg2, model)
    if form.isa(Or):
        return interpretForm(form.arg1, model) or interpretForm(form.arg2, model)
    if form.isa(Implies):
        return not interpretForm(form.arg1, model) or interpretForm(form.arg2, model)
    raise Exception("Unhandled: %s" % form)
        ...
        if len(freeVars) > 0:
            ...
            for obj in objects:
                response[(var, obj)] = self.query(
                    substituteFreeVars(form, var, obj), modify
                )
            return response
