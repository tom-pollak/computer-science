...
1. `fst`, `snd`: fst snd of a tuple
2. `curry`, `uncurry`: adds / removes lispy brackewts
3. `succ`, `pred`: successor and predeccory of an Ord
4. `negate`, `abs`, `signum`:
5. `quot`, `rem`, `div`, `mod`, `divMod`: // remainder / % (/, %)
6. `gcd`: greatist common denominator
7. `id`, `const`, `(.)`: (.) function compasition kinda like a pipe
8. `flip`, `($)`: flips the arguments, $ has a low right binding right associationve precedence so f $ g $ h x = f (g (h x))
9. `error`, `undefined`: stop exectution and displays error, special error case
10. `map`, `(++)`, `filter`: map a list, append two lists,
11. `head`, `tail`, `last`, `init`: head tail, last init every element except last one, opposite of tail
12. `(!!)`, `null`: list index
13. `length`, `reverse`: length
14. `foldr`, `foldr1`, `foldl`, `foldl1`, foldr1: no base case so cannot be applied to empty structures
14. `scanl`, `iterate`, `repeat`, `replicate`, `cycle`: from an initial value it folds left, lazy iteration of repreated applications of f to x iterate f x == [x, f x, f (f x)], infinte list of x, cycle list infinitely
15. `take`, `drop`, `takeWhile`, `dropWhile`, `break`, `splitAt` takes the prefix length n, returns suffix after the first n elements, take wihile condition is tru
16. `zip`, `zipWith`, `unzip`, returns tuples of 2 lists, specifies the separator, unzips
17. `lines`, `words`, `unlines`, `unwords`: breaks string by newline, by space, creates string from list oppositie
